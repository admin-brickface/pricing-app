...
def calculate_stone_sf_or_lf(width, height, item_type='flats'):
    """Calculate SF or LF for stone veneer based on item type"""
    ...
    else:  # corners or sills
        # LF = width / 12
        return width / 12

def calculate_totals(service):
    """Calculate totals for a service"""
    totals = {}
...
def calculate_pricing_tiers(subtotal, add_repair=False, add_rigging=False):
    """Calculate cascading discount tiers"""
...
def generate_pdf(service, totals, pricing, customer_info):
    ...
    title_style = ParagraphStyle(
        ...
        fontSize=24,
        textColor=colors.HexColor('#2E5CB8'),
        spaceAfter=30,
        alignment=1
    )
    ...
    info_data = [
        ...
        ['Address:', customer_info['project_address']],
        ['Sales Rep:', customer_info['sales_rep']],
    ...
    info_table.setStyle(TableStyle([
        ...
        ('TEXTCOLOR', (0, 0), (0, -1), colors.HexColor('#2E5CB8')),
        ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
        ('VALIGN', (0, 0), (-1, -1), 'TOP'),
...
with tabs[0]:
    ...
    gutters_df = st.data_editor(
        ...
        column_config={
            ...
            "LF": st.column_config.NumberColumn("LF", format="%.2f")
        },
        hide_index=True,
...
with tabs[2]:
    ...
    for item, details in SERVICE_DATA['stucco']['misc_items'].items():
        with misc_cols[misc_index % 2]:
            quantity = st.number_input(
                f"{item} ({details['unit']}) - ${details['price']:.2f}",
                min_value=0.0,
                value=st.session_state.misc_quantities.get(item, 0.0),
                step=1.0,
                key=f"misc_{item}"
            )
    ...
    minimums_df = pd.DataFrame([
        ['LOXON PAINTING', f"${SERVICE_DATA['stucco']['minimums']['LOXON PAINTING']:,}"],
        ['CAULKING', f"${SERVICE_DATA['stucco']['minimums']['CAULKING']:,}"]
    ], columns=['Service', 'Minimum Amount'])
    ...
    if st.button("Calculate Stucco Painting", key="calc_stucco"):
        ...
        # Add miscellaneous items to totals
...
with tabs[3]:
    ...
    painting_df = st.data_editor(
        ...
        column_config={
            ...
            "SF": st.column_config.NumberColumn("Quantity/SF", format="%.2f")
        },
        hide_index=True,
        use_container_width=True,
        key="painting_editor"
    ...
    st.subheader("Service Minimums")
    minimums_df = pd.DataFrame([
        ['HOUSE PAINTING', f"${SERVICE_DATA['painting']['minimums']['HOUSE PAINTING']:,}"]
    ], columns=['Service', 'Minimum Amount'])
    st.dataframe(minimums_df, use_container_width=True, hide_index=True)
...
# Customer Information and PDF Generation
st.markdown("---")
